...
const PaymentVerifyPage = () => {
  ...
  const [countdown, setCountdown] = useState(3600); // redirect countdown in seconds
  ...
  useEffect(() => {
    if (status === "success") {
      if (countdown === 0) navigate("/dashboard/attendee/my-tickets");
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      setDownloading(true);
      const response = await api.get(`/tickets/${ticket._id}/download/pdf`, { responseType: "blob" });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">
            {status === "verifying" ? "Verifying your payment..." : "Processing confirmation..."}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-10 rounded-2xl shadow max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">We couldn't confirm your payment. Try again or contact support.</p>
          <button onClick={() => navigate(-1)} className="w-full bg-gray-200 py-3 rounded-xl mb-3">Try Again</button>
          <button onClick={() => navigate("/")} className="w-full bg-purple-600 text-white py-3 rounded-xl">Back to Home</button>
        </div>
      </div>
    );
  }
  ...
  // SUCCESS
  return (
    <div className="w-full min-h-screen py-10 bg-gray-100 bg-cover bg-center" style={{ backgroundImage: `url(${BackgroundImage})` }}>
        ...
        <div className="w-[80%] max-w-md bg-white p-8 rounded-lg shadow-xl">
          ...
          {ticket?.event?.title && (
            <div className="mb-4 border-b pb-4 space-y-2">
              ...
              <span className="mt-2 text-gray-500 text-lg">{new Date(ticket.event.date).toLocaleDateString()} | {ticket.event.location}</span>
            </div>
          )}

          <div className="space-y-3 text-sm">
            {ticket?.tickets?.map((t: any, idx: number) => (
          ...
          <div className="mt-6">
            <button onClick={handleDownloadPDF} disabled={downloading} className="w-full bg-[#4B22A7] text-white py-2 rounded-sm font-semibold">
